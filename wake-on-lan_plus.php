...
function endWithJsonResponse($responseData, $filename = null)
{
    ...
    extracted($responseData, $filename);
}/**
 * @param $responseData
 * @param $filename
 * @return void
 */function extracted($responseData, $filename)
{
...
if ("CONFIG.GET" === $ajaxOperation) {
    ...
    $MESSAGE = wakeOnLan($mac, safeGet($_GET, "host", ""), safeGet($_GET, "cidr", ""), safeGet($_GET, "port", ""), $debugOut);
    ...
    if ($MESSAGE) {
        ...
        endWithJsonResponse(["info" => "Magic packet has been sent for <strong>" . $mac . "</strong>. Please wait for the host to come up...",]);
    }
} else {
    if (isset($_GET["aop"])) {
            ...
            updateElements = function (lang, data) {
                ...
                $('[data-lang-ckey],[data-lang-tkey],[data-lang-pkey]').each(function () {
                    var
                        ...
                        pval         // Value of the placeholder attribute
    ...
    $(function () {
        ...
        var
            ...
            , saveConfigToServer = function () {
                $('#hostTable tbody tr').map(function () {
                    return $(this).data('wol');
                }).get();
                $.ajax({
